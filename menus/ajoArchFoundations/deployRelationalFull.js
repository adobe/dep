...
export async function deployRelationalFull() {
  try {
    ...
    // Phase 1: Deploy relational schemas & datasets
    try {
      await deploySchemaModel(deployYamlPath, envMap, accessToken, { type: "relational" });
    } catch {
      return; // deploySchemaModel already logged the error
    }

    // Phase 2: Verify relational datasets active
    console.log("\n  Checking relational dataset status...");
    ...
    if (pending.size === 0) {
      ...
      for (const [name] of notEnabled) {
        try {
          ...
          enableFailed.push(name);
        }
      }
      if (enableFailed.length > 0) {
        console.log(chalk.red("  ✗") + ` ${enableFailed.length} dataset(s) could not be enabled — polling will confirm status:`);
        for (const name of enableFailed) {
          console.log(chalk.red("    ✗") + `  ${name}`);
        }
      }
