...
export async function cleanSandbox() {
  try {
    ...
    const [depFlows, depConnections, allSegments, allPolicies, inspection] = await Promise.all([
      listFlows(accessToken, envMap, "standard"),
    ...
    if (inspection.locked.length > 0) {
      ...
      for (const item of inspection.locked) {
        console.log(`    ${chalk.yellow("⚠")}  ${item.title.padEnd(titleWidth)}  ${chalk.yellow(`(${item.reason})`)}`);
      }
      console.log();
    }
    ...
    if (depSegments.length > 0) {
      ...
      console.log((allOk ? chalk.green("  ✓") : chalk.yellow("  !")) + `  Audiences (${deletedSegCount}/${depSegments.length})`);
    }
    ...
    if (depPolicies.length > 0) {
      ...
      console.log((allOk ? chalk.green("  ✓") : chalk.yellow("  !")) + `  Merge policies (${deletedPolCount}/${depPolicies.length})`);
    }
    ...
    for (const flow of depFlows) {
      try {
        ...
        let connectionsClean = true;

        if (flow.targetConnectionId) {
          if (!await deleteTargetConnection(flow.targetConnectionId, accessToken, envMap)) {
            connectionsClean = false;
          }
        }

        if (flow.sourceConnectionId) {
          if (!await deleteSourceConnection(flow.sourceConnectionId, accessToken, envMap)) {
            connectionsClean = false;
          }
        }
        ...
        deletedFlowCount++;
        if (!connectionsClean) {
          flowErrors.push(`"${flow.name}" — flow removed but some connections may remain`);
        }
      } catch (err) {
    ...
    if (depFlows.length > 0) {
      ...
      console.log((allOk ? chalk.green("  ✓") : chalk.yellow("  !")) + `  Dataflows (${deletedFlowCount}/${depFlows.length})`);
    ...
    if (depConnections.length > 0) {
      ...
      console.log((allOk ? chalk.green("  ✓") : chalk.yellow("  !")) + `  Source accounts (${deletedConnCount}/${depConnections.length})`);
