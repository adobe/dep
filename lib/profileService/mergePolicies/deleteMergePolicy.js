 ...
 * @param {string} id - The merge policy ID to delete.
 * @returns {Promise<true>} Resolves to true on success.
 */
export async function deleteMergePolicy(accessToken, envMap, id) {
  const { API_KEY: apiKey, IMS_ORG: imsOrg, SANDBOX_NAME: sandbox } = envMap;

  const url = `https://platform.adobe.io/data/core/ups/config/mergePolicies/${id}`;
  const headers = {
  ...
  return withRetry(async () => {
    ...
    throw new Error(`Failed to delete merge policy ${id}: ${detail}`);
  }, 3, 2000);
}
