...
/**
 * Deletes a mapping set by ID via the Conversion API.
 * Endpoint investigation returned HTTP 200 for DELETE /data/foundation/conversion/mappingSets/{id}.
 *
 ...
 * @param {string} id - The mapping set ID to delete.
 * @returns {Promise<true>} Resolves to true on success.
 */
export async function deleteMappingSet(accessToken, envMap, id) {
  const { API_KEY: apiKey, IMS_ORG: imsOrg, SANDBOX_NAME: sandbox } = envMap;

  const url = `https://platform.adobe.io/data/foundation/conversion/mappingSets/${id}`;
  const headers = {
  ...
  return withRetry(async () => {
    ...
    throw new Error(`Failed to delete mapping set ${id}: ${detail}`);
  }, 3, 2000);
}
