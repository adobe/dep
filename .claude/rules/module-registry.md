...
and module-specific exceptions. Referenced by `cli-verify`, `cli-menu`, and `CLAUDE.md`.
...
- **`cli-verify` excludes `sandboxManagement` from its verification subagent scope** — there
  are no deployable AEP/AJO artifacts in this module to check.
...
- `cli-verify` subagent cleanup phase
